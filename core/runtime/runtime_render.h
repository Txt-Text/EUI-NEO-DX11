...
namespace core::dsl {
	...
	inline void Runtime::prepareTextElement(
		...
		const auto retainedBounds = paintBounds_.find(element.id);
		if (retainedBounds != paintBounds_.end() && retainedBounds->second.hasSubtree) {
			const Rect subtreePixels = toPixelRect(retainedBounds->second.subtree, dpiScale);
			const Rect retainedBoundsPixels = core::render::clampRenderRect(subtreePixels, windowWidth, windowHeight);
			if (isRetainedLayerCandidate(element,
				retainedBounds->second,
				subtreePixels,
				dirtyRect,
				effectiveHasScissor,
				effectiveScissor)) {
				const auto layerIt = retainedLayers_.find(element.id);
				if (layerIt != retainedLayers_.end() &&
					layerIt->second.valid &&
					layerIt->second.handle != nullptr &&
					layerIt->second.signature == retainedLayerSignature(element, retainedBounds->second, dpiScale) &&
					closeEnough(layerIt->second.bounds, retainedBoundsPixels) &&
					layerIt->second.width == static_cast<int>(std::ceil(retainedBoundsPixels.width)) &&
					layerIt->second.height == static_cast<int>(std::ceil(retainedBoundsPixels.height))) {
					return;
				}
			}
		}
	...
	inline std::string textLayoutSyncKey(const Element& element) {
		return element.dirtyKey + "|" +
			element.text + "|" +
			element.fontFamily + "|" +
			std::to_string(element.fontSize) + "|" +
			std::to_string(element.fontWeight) + "|" +
			std::to_string(element.maxWidth) + "|" +
			(element.wrap ? "1" : "0") + "|" +
			std::to_string(static_cast<int>(element.horizontalAlign)) + "|" +
			std::to_string(static_cast<int>(element.verticalAlign)) + "|" +
			std::to_string(element.lineHeight);
	}

	inline std::string textPaintSyncKey(const Element& element,
		const RenderTransform& renderTransform,
		float dpiScale) {
		auto quant = [](float value) {
			return std::to_string(static_cast<long long>(std::llround(value * 4096.0f)));
			};

		const auto frame = element.frame;
		return quant(frame.x) + "|" +
			quant(frame.y) + "|" +
			quant(frame.width) + "|" +
			quant(frame.height) + "|" +
			quant(element.opacity) + "|" +
			quant(element.textColor.r) + "|" +
			quant(element.textColor.g) + "|" +
			quant(element.textColor.b) + "|" +
			quant(element.textColor.a) + "|" +
			quant(renderTransform.opacity) + "|" +
			quant(dpiScale) + "|" +
			quant(element.transform.translate.x) + "|" +
			quant(element.transform.translate.y) + "|" +
			quant(element.transform.scale.x) + "|" +
			quant(element.transform.scale.y) + "|" +
			quant(element.transform.rotate);
	}
	...
	inline void Runtime::prepareText(
		...
		const std::string layoutKey = textLayoutSyncKey(element);
		if (instance.layoutSyncKey == layoutKey) {
			return;
		}
		instance.layoutSyncKey = layoutKey;
