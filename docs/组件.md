...
当前 input 支持 focus、点击定位光标、拖动选择、Shift+方向键选择、Ctrl+A/C/X/V、Backspace、Delete、Enter、单行和多行模式，并会把系统 IME 候选框锚定到当前光标位置。光标、选择、删除和横向滚动基于 `TextPrimitive::measureTextMetrics(...)` 的 shaped caret stops，适配 emoji、CJK、阿拉伯连字和组合符号。默认字体族是跨平台 `monospace`，默认占位文本是 `Hello EUI-NEO 😉`。暂时没有 IME 组合态显示和撤销栈。
