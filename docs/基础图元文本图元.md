...
FreeType 和 HarfBuzz 源码随项目放在 `3rd/freetype`、`3rd/harfbuzz`，默认由 CMake 一起构建。HarfBuzz 默认开启，可在配置时通过 `-DEUI_ENABLE_HARFBUZZ=OFF` 关闭；关闭后会退回基础逐 codepoint fallback shaping，复杂脚本和连字能力会降低。
...
`core::TextPrimitive::measureTextMetrics(text, fontFamily, fontSize, fontWeight)` 返回 shaped 文本宽度和 byte index 到 caret x 的停靠点。输入框、复杂文本命中测试和需要精确光标定位的组件应使用这份 metrics，而不是对子串重复测宽。
