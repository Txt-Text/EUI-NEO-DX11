...
- `auto`：默认。Vulkan SDK 存在且 EUI Vulkan backend 已实现时解析为 Vulkan，否则解析为 OpenGL。
- `opengl`：强制 OpenGL。
- `vulkan`：强制 Vulkan；Vulkan SDK 不存在或 EUI Vulkan backend 未实现时配置失败，不静默回退。
...
这里的“可用”同时要求系统 Vulkan SDK/loader 可用，以及 EUI 自身 Vulkan backend 已实现并可初始化。只检测到 SDK 不等于后端可用。
...
- `EUI_RENDER_BACKEND=auto` 在 Vulkan 可用时解析为 Vulkan，不可用时解析为 OpenGL。
