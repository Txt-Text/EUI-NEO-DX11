...
- Vulkan primitive、text、image 和 render cache 已开始接入真实绘制路径；稳定 UI 绘制必须继续按阶段 5 收束 CPU/GPU 资源边界并做 OpenGL/Vulkan 视觉验收。
...
- 图片解码、GIF 帧、远程缓存已由 `core/render/image_source.*` 共用；OpenGL/Vulkan image 后端只负责 texture 上传、更新和 draw。
...
- Vulkan atlas/texture/draw/cache 留在 `core/render/vulkan/*`。
...
- OpenGL/Vulkan gallery 手测无明显文本、图标和图片缺失后，再继续对 render cache 和资源生命周期做压力验证。
...
- OpenGL/Vulkan 重复的图片解码、GIF 和远程缓存逻辑已收束到共享 `core/render/image_source.*`。

### 阶段 9：Vulkan render cache 和生命周期压测

完成标准：

- Vulkan `ensureRenderCache` 创建 offscreen color image、image view 和 framebuffer，不允许 no-op。
- dirty rect 重绘写入 render cache，再 copy/blit 到 swapchain。
- cache 渲染期间 backdrop blur 从 cache image snapshot 取样，直接渲染时继续从 swapchain snapshot 取样。
- render cache、text/image descriptor、pipeline、buffer 和 texture 资源在 swapchain recreate、窗口关闭和 image source churn 时释放顺序明确。
- `vulkan_lifecycle` 可以在 OpenGL/Vulkan 下构建运行，用于手动 resize、长跑和资源释放压测。

当前状态：

- Vulkan render cache 第一段已接入 offscreen image/framebuffer 和 cache-to-swapchain copy。
- `examples/vulkan_lifecycle.cpp` 已新增，覆盖 primitive/text/image churn、GIF 帧更新和 image descriptor 申请释放。
- 主窗口 shutdown、托盘隐藏和子窗口销毁会在 scoped active render backend 下释放 DSL 图形资源，避免 Vulkan texture/descriptor 释放时找不到后端。
- Vulkan image texture 销毁已改为 fence 后延迟释放，避免 DSL update 阶段释放上一帧仍可能被 GPU 使用的 descriptor/image。
- 仍需专项手测 resize、最小化、多窗口、modal 和 SDL2 边缘行为。
...
cmake --build --preset vulkan-glfw-release --target vulkan_lifecycle --parallel
```
